    ...
    createGraph(
      ...
      originNodes.forEach((node, index) => {
        nodePosition[node.id] = {
          x: index === 0 ? centerX : centerX + radius * Math.sin(dagre * (index - 1)),
          y: index === 0 ? centerY : centerY + radius * Math.cos(dagre * (index - 1)),
        };
      });
      ...
      animateTexts.forEach((text, index) => {
        text
          ...
          .delay(index === 0 ? 0 : 2 * duration) // 一次展开
          .attr('fill-opacity', 1);
      });
